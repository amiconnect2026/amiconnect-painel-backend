...
router.post('/pausar', async (req, res) => {
  try {
    ...
    const result = await pool.query(`
      ...
      SELECT id, $2, 'humano', $3::varchar, NOW() + INTERVAL '10 hours',
        CASE WHEN $3::text = 'pos_venda' THEN NOW() ELSE NULL END
      FROM clientes
      WHERE telefone = $1
        AND empresa_id = $2
      ON CONFLICT (cliente_id, empresa_id) DO UPDATE SET
        status = 'humano',
        origem_pausa = $3::varchar,
        pausado_ate = NOW() + INTERVAL '10 hours',
        pos_venda_contatado_em = CASE WHEN $3::text = 'pos_venda' THEN NOW() ELSE sessions.pos_venda_contatado_em END,
        updated_at = NOW()
